...
export async function GET(
  ...
  try {
    ...
    console.error(`Error fetching invoice data for ${await (await params).id}:`, error);
...
export async function PATCH(
  ...
  try {
    ...
    const { headers, data, status } = body;
    
    const docRef = adminDb.collection(COLLECTIONS.INVOICE_UPLOADS).doc(id);
    const docSnap = await docRef.get();
    
    if (!docSnap.exists) return NextResponse.json({ error: "Record not found" }, { status: 404 });
    const existingData = docSnap.data();

    // Update Content if provided
    if (headers && data) {
       const storagePath = existingData?.storagePath;
       if (!storagePath) return NextResponse.json({ error: "No storage path to update" }, { status: 400 });
