...
รอบนี้ทำกลุ่ม **A + B + C** เสร็จ (D และ E ตามข้อตกลง: D เลื่อนไว้, E-2e ตีความว่า "มีโปรหลายตัวพร้อมกัน" = เสร็จแล้ว ไม่ต้องทำเพิ่ม) — ทุก task ผ่าน review (spec + quality) และ `tsc --noEmit` สะอาด
...
**ยังเหลือ (ยังไม่ทำในรอบนี้):** กลุ่ม D ทั้งหมด (รายงาน Supervisor — ฟีเจอร์ใหม่ต้องออกแบบ) · ข้อ 3b "จำนวนสินค้า" มีแล้ว/ "จำนวนบิล" เพิ่มแล้ว · validate โปรซ้อนทับช่วงวันที่ในหน้าหลังบ้าน (P2 #8 ยังไม่ทำ)
> ⚠️ ก่อน merge ควรทดสอบจริงบนอุปกรณ์/เว็บ (repo ไม่มี test suite — verify ด้วย tsc + review เท่านั้น)
...
| ฟีเจอร์ | หัวข้อในใบเสนอราคา | สถานะรวม |
| --- | --- | --- |
| 1 | บันทึกยอดขายผ่าน Mobile App | 🟡 ~60% (มี **บัคขวาง production**) |
| 2 | ราคาตามช่วงโปรโมชั่น | 🟡 ~75% (ขาดโปรหลายสินค้า) |
| 3 | รายงานยอดขายของพนักงานขาย | 🟡 ~55% (ขาด filter พนักงาน + PDF) |
| 4 | รายงานยอดขายของ Supervisor | ❌ 0% (ยังไม่มี) |
| 5 | รายงานยอดขายทุกสาขา | 🟡 ~30% (หน้ายังเป็น **ข้อมูลปลอม/mock**) |

> **ประเด็นที่ต้องรีบที่สุด:** ฟีเจอร์ 1 มีบัคที่ทำให้ **บันทึกยอดขายบน production ไม่ได้เลย** (Firestore rules บล็อก) — ต้องแก้ก่อนส่งมอบ
...
- [ ] สร้างหน้ารายงานยอดขายของทีมที่ supervisor ดูแล (scope ตาม `managedBranchIds`/ลูกทีม) *(4a)*
- [ ] แสดงยอดรวมทีม + สถิติรายสาขา + เปรียบเทียบระหว่างสาขา *(4b, 4c)*
- [ ] เพิ่ม Excel/PDF export ของรายงาน supervisor *(4d)*
