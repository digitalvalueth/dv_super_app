...
> สถานะ: วิเคราะห์แล้ว — พร้อมเริ่มพัฒนา
...
- Lock ห้ามอัปโหลดรูป 2 วัน ในวันที่ 1 และ 16 (เป็นวันตัดรอบ)
- Grace period ลับ +5 วัน หลังสิ้นรอบ
...
interface CountingPeriod {
  ...
  startDate: Timestamp; // เช่น 1 มี.ค.
  endDate: Timestamp; // เช่น 15 มี.ค.
  lockDates: Timestamp[]; // วันที่ห้าม upload (1, 16)
...
- Functions:
  ...
  - `canUploadPhoto(userId, periodId)` → ตรวจสอบว่าอยู่ใน lock date ไหม
  - `isInGracePeriod(periodId)` → ตรวจสอบว่าอยู่ใน grace period ไหม (ไม่แสดงให้พนักงานรู้)
  - `closePeriod(periodId)` → ปิดรอบ + ส่งข้อมูลเข้า ITP
  - `generatePeriods(companyId, year)` → สร้างรอบทั้งปี

##### C. Mobile App Changes

- ก่อนถ่ายรูป → เช็ค `canUploadPhoto()` → ถ้า lock → แสดง "ระบบปิดรับรูปชั่วคราว"
- **ห้ามแสดงเหตุผลเรื่อง grace period** → พนักงานเห็นแค่ "หมดเวลาส่งรูป" (ถ้าเลย endDate) แต่ระบบยัง accept ได้ถึง graceEndDate
- แสดง badge "รอบปัจจุบัน: 1-15 มี.ค." ที่หน้า stock-counter

##### D. Web Dashboard Changes

- เพิ่มหน้า period management สำหรับ admin
- แสดง stats แยกตามรอบ
- **ปุ่ม "Request ขยายเวลา"** สำหรับ supervisor (ตามที่ประชุมพูดถึง)

**ประมาณการ**: 5-7 วัน

---

#### 2.2 แก้ไขยอดนับบน Web (Supervisor Override)

**ความต้องการ**: เมื่อ AI นับได้ 8 แต่ BA นับได้ 11 → Supervisor สามารถเลือกตัวเลขที่จะใช้จริงได้
...
interface CountingSession {
  // ... existing fields
  finalCount?: number; // ตัวเลขสุดท้ายที่ supervisor approve
  supervisorOverride?: {
    overriddenBy: string; // supervisor userId
    overriddenAt: Timestamp;
    previousCount: number; // ค่าก่อนแก้
    newCount: number; // ค่าใหม่
    reason: string; // เหตุผลที่แก้
  };
...
- เพิ่ม:
  - ปุ่ม "แก้ไขยอด" ข้างแต่ละ counting session
  - Modal: แสดง AI Count, BA Count (dispute), ช่อง Final Count + Reason
  - สถานะ: Pending Review → Approved / Rejected
  - Filter ตามสถานะ (รอตรวจ, อนุมัติแล้ว, แก้ไขแล้ว)
...
### คำถามสำหรับทีมพัฒนา (อั้น)

1. **Lock 2 วัน หมายความว่าอย่างไรกันแน่?**
   - ห้ามอัปโหลดรูปในวันที่ 1 และ 16 เลย? หรือห้ามอัปโหลดรูปของรอบก่อนหน้า?
   - เช่น วันที่ 1 มี.ค. → ห้ามอัปโหลดรูปของรอบ 16-28 ก.พ.? หรือห้ามอัปโหลดรูปทั้งหมด?
   - **ข้อเสนอ**: Lock เฉพาะรอบก่อนหน้า → พนักงานยังถ่ายรูปรอบใหม่ได้

2. **Grace period +5 วัน — ทำงานยังไงเมื่อข้ามรอบ?**
   - รอบ 1-15 มี.ค. + grace 5 วัน = ถ่ายได้ถึง 20 มี.ค.
   - แต่ 16-20 มี.ค. ก็เป็นรอบใหม่ (16-31 มี.ค.) ด้วย
   - **ข้อเสนอ**: ใช้ `periodId` แยกชัดเจน → ถ่ายรูปของรอบเก่าได้ แต่ต้องระบุว่าเป็นรอบไหน
   - **UI**: พนักงานเห็น "หมดเวลาส่งรูป" หลังวันที่ 15 → แต่ถ้ากดส่ง ระบบยัง accept แบบเงียบๆ พร้อมทำเครื่องหมาย "ส่งล่าช้า"

3. **Export ITP — ตอนไหนส่ง?**
   - อัตโนมัติเมื่อปิดรอบ?
   - หรือ Manual กดปุ่ม "ส่ง ITP" โดย admin?
   - **ข้อเสนอ**: Manual + ปุ่ม "Request" ตามที่ประชุมบอก → admin ตรวจสอบก่อนส่ง

4. **Qty + Price ที่ต้อง export**
   - Price มาจากไหน? ตอนนี้ Product model ไม่มี field `price`
   - ต้องเพิ่ม Price ที่ไหน? Product master? หรือ branch-level pricing?
   - **ข้อเสนอ**: เพิ่ม `price` ใน Product model + import price list จาก Watson module ที่มีอยู่แล้ว

### คำถามสำหรับลูกค้า (คุณหมวย)

1. สินค้า 100+ ชิ้นมีตัวไหนบ้าง? กี่ SKU?
2. Format import ของ ITP เป็นอย่างไร? (Excel? CSV? API?)
3. Qty + Price → Price ใน ITP มาจากไหน? Cost price หรือ Selling price?
4. สาขาทั้งหมดมีกี่สาขา? อยากได้ Excel ที่มี: ชื่อสาขา, code, ที่อยู่, พนักงาน BA, supervisor
5. Project ใหม่ที่ต้อง upload บิล — เป็นบิลอะไร? ใบเสร็จ? ใบส่งของ? บิลซื้อ?
...
สัปดาห์   3-4:  Phase 2a — ระบบตัดรอบ + grace period
สัปดาห์   5-6:  Phase 2b — แก้ไขยอดบน web + แจ้งเตือนไม่เช็คอิน + ถ่ายเพิ่ม
สัปดาห์   7-9:  Phase 3a — Prompt Management
สัปดาห์  10-12: Phase 3b — ITP Integration (เมื่อได้ format)
สัปดาห์  13+:   Phase 4 — Multi-photo, Bill upload
```
...
| 🔵 P3 | ITP Integration            | รอ format ก่อน                  |
...
| ไฟล์                                                            | คำอธิบาย                  |
| --------------------------------------------------------------- | ------------------------- |
| `services/counting-period.service.ts`                           | จัดการรอบการนับ           |
